...
class WXRangeDownload(
    ...
    private lateinit var file: WXSafeRandomAccessFile // 存放的文件
    private lateinit var tempFile: WXSafeRandomAccessFile // 指针文件
    ...
    suspend fun runDownload(downloadNet: WXDownloadNet) {
        try {
            file = WXSafeRandomAccessFile(downLoadFileBean.saveFile.absolutePath)  // 存放的文件
            tempFile = WXSafeRandomAccessFile(downLoadFileBean.tempFile.absolutePath)  //指针文件
        } catch (e: Exception) {
            e.printStackTrace()
        }
